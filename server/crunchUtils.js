...
/**
 * Calculates standard print duration for a project based on materials.
 */
function getProjectPrintDays(mats) {
...
/**
 * Reverse-calculates crunched timeline from targetLaunchDate.
 * Distributes required compression backwards across uncompleted stages.
 */
function calculateCrunchedTimeline(project, targetLaunchDate) {
  ...
  // Determine standard print days
  const stdPrintDays = getProjectPrintDays(materials);
  const minPrintDays = Math.max(7, Math.round(stdPrintDays * 0.65)); // can compress up to 35%
  ...
  // Identify stages to be processed (from currentStage forward to Connectivity)
  ...
  // Calculate standard remaining days
  ...
  // Available days between now (or briefDate if at Brief) and targetLaunchDate
  ...
  // If available days >= standardRemainingDays, no crunch needed
  ...
  // Distribute effectiveDaysToCrunch backwards across remaining stages proportionally to max crunchable days
  ...
  if (totalCrunchable > 0) {
    // Reverse stage order distribution
    ...
    for (const s of revStages) {
      ...
      // Allocate proportional share with a floor, capped by maxCanCrunch
    ...
    // If rounding left any remainder, sweep backwards
  ...
  // Calculate new crunched milestone dates sequentially
  const crunchedMilestones = { Brief: briefDate };
  let cursor = startDate;

  // Preserve already completed stages from existing project milestones
